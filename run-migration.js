...
async function createResourcesTable() {
  ...
  try {
    ...
    // let's use a different approach - check if table exists first
    ...
    if (error && error.message.includes('relation "public.resources" does not exist')) {
      console.log('Resources table does not exist. Please run the migration manually.');
...
createResourcesTable()
  .then(success => {
    if (success) {
      ...
      console.log('Migration failed or table already exists.');
      process.exit(1);
    }
  })
  .catch(err => {
