            ...
            <div className="h-full p-4">
              <div className="max-w-4xl mx-auto space-y-6">
                <div className="text-center">
                  ...
                  <p className="text-muted-foreground">Manage system settings and AI configurations</p>
                </div>
                
                {/* Admin Panel - AI Endpoint Configuration */}
                <Card className="p-6">
                ...
                <Card className="p-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <SystemMonitorCard 
                      ...
                      value="245 requests" 
                      status="normal" 
                      description="Today's API usage across all endpoints" 
                    />
                    <SystemMonitorCard 
                      title="Response Time" 
                      value="342ms avg" 
                      status="fast" 
                      description="Average response time across all providers" 
                    />
                    <SystemMonitorCard 
                      title="System Health" 
                ...
                <Card className="p-6">
                  ...
                  <div className="space-y-4">
                    <div className="flex justify-between items-center">
                      ...
                      <Button variant="outline" onClick={() => console.log('Testing all providers')}>
                        Run Health Check
                      </Button>
                    </div>
                  </div>
                ...
                {/* Admin Panel - Unified Model Selection */}
                <Card className="p-6">
                  <h3 className="text-lg font-semibold mb-4 flex items-center gap-2">
                    <Zap className="h-5 w-5 text-blue-500" />
                    Unified Model Response Arrangement
                  </h3>
                  <div className="space-y-4">
                    <p className="text-sm text-muted-foreground">
                      Select and arrange responses from multiple AI models to create unified context
                    </p>
...
interface EndpointConfigurationCardProps {
  endpoint: string;
  label: string;
  value: string;
  model: string;
...
function EndpointConfigurationCard({ 
  endpoint, 
  label, 
  value, 
  model,
  onProviderChange,
  onModelChange
}: EndpointConfigurationCardProps) {
  return (
    <Card className="p-4">
...
interface SystemMonitorCardProps {
  title: string;
  value: string;
  status: 'healthy' | 'warning' | 'error' | 'normal' | 'fast';
  description: string;
}

function SystemMonitorCard({ title, value, status, description }: SystemMonitorCardProps) {
  const statusColors = {
  ...
  const statusIcons = {
    ...
    normal: <Star className="h-4 w-4 text-blue-500" />,
...
// Endpoint Configuration Tab Component
const EndpointConfigurationTab = ({ userId }: { userId: string }) => {
  const { fetchProfileData } = useStudyBuddy();
  const [settings, setSettings] = useState<StudyBuddySettings>(defaultUserSettings.studyBuddy);

  // Load settings for the user
  useEffect(() => {
    const loadSettings = async () => {
      // In a real implementation, you'd fetch the user's settings from the database
      // For now, we'll use the default settings
      setSettings(defaultUserSettings.studyBuddy);
    };
  ...
  const handleSettingsChange = (newSettings: StudyBuddySettings) => {
  ...
  return (
    <div className="space-y-4">
      ...
      <EnhancedProviderSelector 
        endpoint="personalized" 
        label="Personalized Endpoint" 
        settings={settings}
        onSettingsChange={handleSettingsChange}
      />
      <EnhancedProviderSelector 
        endpoint="semanticSearch" 
        label="Semantic Search Endpoint" 
        settings={settings}
        onSettingsChange={handleSettingsChange}
      />
      <EnhancedProviderSelector 
