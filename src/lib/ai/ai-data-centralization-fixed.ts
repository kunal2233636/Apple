...
export class AICentralizedDataService {
  ...
  async getAIUserProfile(userId: string, options: AIContextOptions = {}): Promise<AIUserProfile | null> {
    ...
    try {
      ...
      const { data: subjectsData } = await supabaseBrowserClient
        .from('subjects')
        .select(`
          id,
          name,
      ...
      const { data: dailySummaries } = await supabaseBrowserClient
      ...
      const aiProfile: AIUserProfile = {
        ...
        basicInfo: {
          ...
          lastActiveAt: gamificationData?.last_activity_date || profileData.created_at
        },
        gamification: {
          ...
          totalPoints: gamificationData?.total_points_earned || 0,
          badgesEarned: Array.isArray(gamificationData?.badges_earned) 
            ? gamificationData.badges_earned as string[] 
            : [],
          totalTopicsCompleted: gamificationData?.total_topics_completed || 0
        },
        subjects: this.processSubjectsData(subjectsData || []),
        recentActivity: this.processRecentActivity(dailySummaries || []),
  ...
  async getDailyAIContext(userId: string, date: Date): Promise<any> {
    ...
    try {
      ...
      const dailyContext = {
        date: data.date,
        studyTime: data.total_study_minutes || 0,
        blocksCompleted: data.blocks_completed_count || 0,
  ...
  async getRevisionQueueData(userId: string): Promise<any> {
    try {
      ...
      const overdueCount = revisionTopics.filter((rt: any) => rt.due_date < today).length;
      const dueTodayCount = revisionTopics.filter((rt: any) => rt.due_date === today).length;
      const highPriorityCount = revisionTopics.filter((rt: any) => rt.priority === 'high' || rt.priority === 'critical').length;

      return {
        overdueCount,
        dueTodayCount,
        highPriorityCount,
        topicsDue: revisionTopics.slice(0, 10).map((rt: any) => ({
          id: rt.topic_id,
